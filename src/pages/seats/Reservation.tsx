...
export default function Reservation() {
  ...
  // 전체 데이터
  const { data: seatData, isLoading, error } = useAxios<SeatsResponse[]> (
    ['seats'],
    {
      config: {
        url: '/seats',
  ...
  useEffect(()=>{
    if(seatData){
      ...
      // 초기 좌석 데이터
      const initialSeats = initSectionData.reduce((acc, section) => {
  ...
  <div>
    ...
    <div className='flex flex-row my-5'>
      <div className='flex flex-col'>
        ...
        <SectionOfSeats sectionOfSeats={sectionOfSeats} selectedSeats={selectedSeats} setSectionOfSeats={setSectionOfSeats} setSelectedSeats={setSelectedSeats} sectionData={sectionData} setSectionData={setSectionData} selectedSection={selectedSection} />
      </div>
      <div className='flex flex-col'>
